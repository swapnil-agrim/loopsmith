...
-- data_status: dark
-- Consumer contract: SELECT DISTINCT p50_seconds, p85_seconds, excluded_negative_duration_count FROM metric_2 for the two percentile numbers plus the exclusion count (all constant across every row); SELECT goal_id, cycle_time_seconds FROM metric_2 for the raw scatter series (goal_id IS NULL identifies the all-excluded placeholder row, see below).
