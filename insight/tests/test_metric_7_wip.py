...
POST-PR-REVIEW BLOCKING FIX: an independent, author-blind pre-PR review reproduced a real defect
-- gy is claimed by a1 (1/1), then claimed AGAIN by a2 (1/5) with no terminal event released in
between (spec metric #35's own named scenario, "lease contention: goals claimed by 2+ actors").
The original SQL paired each claimed event with its own next terminal event and counted
`count(*)` over open episodes, so one lease-contended goal produced TWO open episodes and read
as 2 units of WIP for one goal -- reproduced live: [0, 1, 2, 3] became [0, 2, 3, 4] once gy was
added, i.e. gy alone contributed +1 to every week from 1/5 onward instead of the correct 0
...
def test_metric_7_weekly_wip_replays_claimed_and_terminal_events(conn):
    ...
    # pre-existing [0, 1, 2, 3] series -- giving [0, 1+1, 2+1, 3+1] = [0, 2, 3, 4].
