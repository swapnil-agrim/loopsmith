 ...
 * test_dash_ic_no_leak.py's own `_assert_carol_absent` helper). */
function assertCarolNeedlesAbsent(body) {
  for (const needle of CAROL_NEEDLES) {
    assert.ok(!body.includes(needle), `carol leaked via ${JSON.stringify(needle)}`);
  }
}
...
async function main() {
  ...
  try {
    ...
    for (const needle of BOB_EXCLUSIVE_NEEDLES) {
      assert.ok(!aliceBody.includes(needle), `bob's own data leaked into alice's page via ${JSON.stringify(needle)}`);
    }
    ...
    for (const needle of ALICE_EXCLUSIVE_NEEDLES) {
      assert.ok(!bobBody.includes(needle), `alice's own data leaked into bob's page via ${JSON.stringify(needle)}`);
    }
    ...
    for (const needle of BOB_EXCLUSIVE_NEEDLES) {
      assert.ok(
        !aliceViaQueryParamBody.includes(needle),
        `?actor=bob leaked bob's own data into alice's page via ${JSON.stringify(needle)}`,
      );
    }
    ...
    for (const needle of BOB_EXCLUSIVE_NEEDLES) {
      assert.ok(
        !aliceViaHeaderBody.includes(needle),
        `X-Actor: bob leaked bob's own data into alice's page via ${JSON.stringify(needle)}`,
      );
    }
